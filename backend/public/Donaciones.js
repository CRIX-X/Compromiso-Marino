document.addEventListener("DOMContentLoaded", () => {
  ...
  import("./firebase.js").then(({ auth }) => {
    ...
    .then(({ onAuthStateChanged, signOut }) => {
      ...
      // Si estás en Render, se recomienda usar la misma URL del dominio
      const API = ""; // vacío = mismo dominio que Render
      ...
      onAuthStateChanged(auth, (user) => {
        if (!user) {
          ...
          if (usuarioSpan) usuarioSpan.textContent = "👤 " + email;
          cargarDonaciones();
        }
      });
      ...
      function validar() {
        ...
        if (!luhn(numeroVal)) { alert("Tarjeta inválida"); return false; }
        if (cvv.length !== 3) { alert("CVV inválido"); return false; }
        if (!org) { alert("Selecciona organización"); return false; }
        if (!monto || monto <= 0) { alert("Monto inválido"); return false; }
        return true;
      }
      ...
      async function cargarDonaciones() {
        try {
          ...
          console.error(err);
          alert("No se pudieron cargar las donaciones (CORS o servidor).");
        }
      }
      ...
      function agregarDonacionLista(d) {
        ...
        li.innerHTML = `${d.NombreCliente} - $${d.MontoDinero} <button class="cancelar">Cancelar</button>`;
        ...
        li.querySelector(".cancelar").onclick = () => {
          ...
          btnSi.onclick = async () => {
            try {
              const res = await fetch(`${API}/donacion/${idAEliminar}`, { method: "DELETE" });
              if (!res.ok) throw new Error("Error al eliminar");
              li.innerHTML = `<span style="color:red;">❌ Cancelado</span>`;
              modal.style.display = "none";
            } catch (err) {
              console.error(err);
              alert("No se pudo cancelar la donación.");
            }
          };
      ...
      form?.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          console.error(err);
          alert("Error al procesar la donación (CORS o servidor).");
        }
      });
